...
레트로 게임의 한글 패치를 처음부터 끝까지 만드는 skill이다. 대상은 ROM 카트리지·CD/GD-ROM·플로피 디스크 매체의 콘솔·PC 게임 전반이며, 범위는 초기 조사(매체·텍스트·폰트·훅·수용량 파악)부터 폰트·인코딩 설계, PoC, 텍스트 추출, 번역, 재삽입, 빌드·패치 생성, 에뮬레이터 검증까지의 전 파이프라인이다. 여기 담긴 전략은 8개 플랫폼에 걸친 11개 선행 한글화 프로젝트에서 추려낸 것이다. 크게 단계별 공통 전략(`references/strategy/`)과 플랫폼별 하드웨어 사실·사례 노트(`references/platforms/`)로 나뉜다. 선행 11종은 모두 컴파일(Compile) 개발의 일본어 원문·텍스트 중심 게임이다. 본문에서 "전형/보통"으로 서술된 패턴 중 일부는 이 엔진 계열의 공통성일 수 있으므로, 타 개발사·타 장르 게임에서는 패턴의 존재 자체를 가설로 취급하고 실측으로 확정한다. 플랫폼 문서의 사례 절은 재사용 가능한 규칙이 아니라 검증된 선례와 위험 신호의 목록이다. 이 SKILL.md는 라우팅과 불변식만 담는다. 실제 판단 기준과 절차는 반드시 해당 참조문서를 읽고 따른다.
...
그래프의 "텍스트 표본 추출 → PoC" 순서는 인코딩·제어코드 확인에 필요한 만큼의 대표 블록만 다루는 것을 뜻한다. 한글 1자 출력 PoC는 전량 추출을 전제하지 않으며 초기 조사의 종결부에서 수행한다. 전량 추출·라운드트립 인프라 완성은 PoC 통과 후가 기본이고, 추출 완료 뒤에는 대표 텍스트 1단위(대사 1블록 등)의 엔드투엔드 패치로 파이프라인을 검증한 다음 본 구현에 들어간다.
...
| PoC | `references/strategy/poc.md` | 한글 1자 화면 출력으로 글리프 공간·인코딩 공간·렌더링 경로를 동시 검증 |
...
- **PoC 통과 전에 본 구현에 착수하지 않는다.** 한글 1자가 실제 게임 화면에 나오기 전까지 전량 번역, 전체 폰트 생성, 엔진 본 패치 같은 되돌리기 비싼 작업은 시작하지 않는다. 표본 추출·문서 골격·빌드 라운드트립처럼 PoC 판단에 필요한 기반 작업은 예외다.
