...
레트로 게임의 한글 패치를 처음부터 끝까지 만드는 skill이다. 대상은 ROM 카트리지·CD/GD-ROM·플로피 디스크 매체의 콘솔·PC 게임 전반이며, 범위는 초기 조사(매체·텍스트·폰트·훅·수용량 파악)부터 폰트·인코딩 설계, PoC, 텍스트 추출, 번역, 재삽입, 빌드·패치 생성, 에뮬레이터 검증까지의 전 파이프라인이다. 크게 단계별 공통 전략(`references/strategy/`)과 플랫폼별 하드웨어 사실·사례 노트(`references/platforms/`)로 나뉜다. **모집단 고지**: 여기 담긴 전략과 사례는 선행 한글화 프로젝트에서 추려낸 것이고, 그 선행 사례는 모두 컴파일(Compile) 개발의 일본어 원문·텍스트 중심 게임이다. 본문에서 "전형/보통"으로 서술된 패턴 중 일부는 이 엔진 계열의 공통성일 수 있으므로, 타 개발사·타 장르 게임에서는 패턴의 존재 자체를 가설로 취급하고 실측으로 확정한다. 플랫폼 문서의 사례 절은 재사용 가능한 규칙이 아니라 검증된 선례와 위험 신호의 목록이다. 이 SKILL.md는 라우팅과 불변식만 담는다. 실제 판단 기준과 절차는 반드시 해당 참조문서를 읽고 따른다.
